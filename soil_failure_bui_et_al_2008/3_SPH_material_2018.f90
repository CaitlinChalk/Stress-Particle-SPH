...
!      ------  Reads basic control data for all problems
...
gid_msh    = 41
gid_res    = 42
init_dat   = 71    !  they will be opened later if there is sigma0 
init_chk   = 72    !  to be read
pts_file = 73
...
read (dat_file,*) text
write(chk_file,*) text
read (dat_file,*)  ntype_solid, nstre, nmats    !  ntype=0 1D, 1 plane s, 2 plane eps, 3 axi, 4 3D, 11 Dam break (1D)
write(chk_file,*)  ntype_solid, nstre, nmats    !  nstre=1 1D, 3 plane, 4 axi and 3D
nstr1 = nstre                                   !  nstr1=1 1D  4 plane  4 axi and 3D

if (ntype_solid == 0) then
    nstr1  = 1
    namaTG = nstre + ndimn
elseif (ntype_solid == 1.or.ntype_solid == 2) then
    nstr1  = nstre
    namaTG = nstre + ndimn
elseif (ntype_solid == 4) then
    nstre  = 6
    nstr1  = 6
    namaTG = nstre + ndimn
elseif (ntype_solid == 10) then			!  Scalar convection
    nstre  = 0
    nstr1  = 0
    namaTG = 1
elseif (ntype_solid == 11) then			!  Scalar convection
    nstre  = 0
    nstr1  = 0
    namaTG = 2
endif
...
!      ------   We will read: 
!               sigma0...       nstr1 components at nodes
...
!                               3.... destn(nstr1)  plastic increment of strain
...
character(60)  text
real (kind=DP) :: stress_ic(nstr1), vars(nint_Vars)
...
integer :: ipoin, iprer, it_curves, j, k, ipts_tcurves,start, bc_seg
...
real (kind=DP) :: dens, Young, Poiss, Yield, Hards, gamma, delta, nvp, Dmatx(nstr1,nstr1)   
...
IF (ntype_eco.gt.1) THEN
	...
	enddo
	if (ntype_Solid==1) then                           !   MOST IMPORTANT
		stress2(nstr1) = Poiss * ( stress2(1) + stress2(2) ) !   Szz obtained here ***  
	!elseif (ntype_Solid==2) then
	!	stress2(nstr1) = 0.0
	endif
...
!               (iii) vector of evp rate (1..nstr1)  (nstr1+1) is accum dev evp strain 

!      Input  is lmat           (= mat type), 
!                evpstn         (= acc dev evp strain)  
!                stress(nstr1)  (note dim is nstr1)

!      Output is vivel (nstr1)  (= accum evp. strain )
!				 debar    (= derivative of acc evp. strain) used in RK4 for sources

implicit none

integer ::  nintvars, ie
real (kind=DP) :: Lintvars(:), avect(nstr1), smean, varj2, sint3, theta, steff
real (kind=DP) :: stress2 (:), vivel(:), devia(nstr1), fdatm0, fdatm, hards 
real (kind=DP) :: frict, root3, yield, devpstn, Young, gamma, delta, nvp, evpstn, sigma_out   


IF (ntype_Solid==0) THEN      !  1D. Compute here, 
                                 !  it is cheaper than using subroutines
       devpstn   = 0.0  
       Young     = props(1, 3)
       fdatm0    = props(1, 7)
       Hards     = props(1, 8)
       ...
       evpstn    = Internal_Vars(1,ie)
       fdatm     = fdatm0 + Hards*evpstn
       sigma_out = stress2(1)- fdatm  
       if (sigma_out.gt.0) then
           devpstn = gamma*(sigma_out/fdatm0)**nvp
       endif
     
       vivel(1) = devpstn
       
ELSEIF (ntype_Solid.ge.1.and.ntype_solid.le.3) THEN
...
!      ------ Invariants for plane stress and strain conditions

if (ntype_Solid.le.3) then
...
!      ------  Invariants for 3D conditions

if (ntype_Solid==4) then

  smean = (stress2(1)+stress2(2)+stress2(3))/3.0

  devia(1) = stress2(1) - smean
  devia(2) = stress2(2) - smean
  devia(3) = stress2(3) - smean
  devia(4) = stress2(4)
  devia(5) = stress2(5)
  devia(6) = stress2(6)
  ...
  varj2 = devia(4)*devia(4) + devia(5)*devia(5) &
                  +           devia(6)*devia(6) &
                  + 0.5*( devia(1)*devia(1)+devia(2)*devia(2)+devia(3)*devia(3))
  varj3 =    devia(1)*devia(2)*devia(3) +   2.*devia(4)*devia(5)*devia(6) &
             - devia(1)*devia(5)*devia(5) - devia(2)*devia(6)*devia(6)    & 
...
!  Obtain plastic vector  df/ds  avect(nstr1)
...
IF (ntype_Solid.le.3) THEN
	...
	If (steff.gt.0) then
	   do istr1=1,nstr1         !      ------  Get vector a2=d(Steff)/ds
...
IF (ntype_solid==4) THEN
    ...
    veca3(1) = devia(2)*devia(3) - devia(5)*devia(5) + varj2/3.
    veca3(2) = devia(3)*devia(1) - devia(6)*devia(6) + varj2/3.
    veca3(3) = devia(1)*devia(2) - devia(4)*devia(4) + varj2/3.
    veca3(4) = 2.*(devia(6)*devia(4) - devia(1)*devia(5))
    veca3(5) = 2.*(devia(4)*devia(5) - devia(2)*devia(6))
    veca3(6) = 2.*(devia(5)*devia(6) - devia(3)*devia(4))
...
real (kind=DP) :: dmatx(nstr1,nstr1), Young, Poiss, const, conss, alfa, beta, G 
...
if (ntype_Solid==0) then                      !      ------  1D solid
    dmatx(1,1) = young

elseif (ntype_Solid==1) then                      !      ------  Plane stress
...
elseif (ntype_Solid==2) then                  !      ------  Plane strain
...
elseif (ntype_Solid==3) then                  !      ------  Axisymmetric
...
elseif ( ntype_Solid==4) then                 !      ------  3D solid
...
elseif (ntype_Solid.ge.1.and.ntype_Solid.le.3) then  ! -- 2D and axial sym situations
...
elseif (ntype_Solid==31) then  ! -- 3D and axial sym situations  

    ddev_strn = sqrt((2.0*(vivel(1)*vivel(1) + vivel(2)*vivel(2)       &
                                             + vivel(3)*vivel(3))      &
                         + vivel(4)*vivel(4) + vivel(5)*vivel(5)       &
                         + vivel(6)*vivel(6))/3.0) 
                         
    dvol_strn = ( vivel(1) + vivel(2) + vivel(3) ) 
   ...
   write(*,*) ' wrong ntype_solid in routine update internal vars '
   write(*,*) ' type CR'
   !pause
   stop
endif
...
if (disp_out == 1) write(node_file,'(A)', advance = 'no') 'disp_10,' !disp_10 is relevant for velocity vector approach, when determining the displacement tolerance (disp_tol)
...
do i = 1,nnode
        ...
        if (disp_out == 1) then
                write(node_file,'(F16.8)', advance = 'no') disp_10(i)
                write(node_file,'(A)',advance = 'no') ','
        end if
        if (rho_out == 1) then
